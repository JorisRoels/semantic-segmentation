...
"""
    This is a script that trains U-Net autoencoder from scratch
    Usage:
...
from networks.unet import Autoencoder2D, Autoencoder3D
...
if args.method == "2D":
    train_xtransform, train_ytransform, test_xtransform, test_ytransform = get_augmenters_2d(augment_noise=False)
else:
    train_xtransform, train_ytransform, test_xtransform, test_ytransform = get_augmenters_3d(augment_noise=False)
if args.data == 'epfl':
    train = EPFLTrainDataset(input_shape=input_shape, preprocess='unit', transform=train_xtransform, target_transform=train_ytransform)
    test = EPFLTestDataset(input_shape=input_shape, preprocess='unit', transform=test_xtransform, target_transform=test_ytransform)
else:
    if args.data == 'embl_mito':
        train = EMBLMitoTrainDataset(input_shape=input_shape, preprocess='unit', transform=train_xtransform, target_transform=train_ytransform)
        test = EMBLMitoTestDataset(input_shape=input_shape, preprocess='unit', transform=test_xtransform, target_transform=test_ytransform)
    else:
        train = EMBLERTrainDataset(input_shape=input_shape, preprocess='unit', transform=train_xtransform, target_transform=train_ytransform)
        test = EMBLERTestDataset(input_shape=input_shape, preprocess='unit', transform=test_xtransform, target_transform=test_ytransform)
...
if args.method == "2D":
    net = Autoencoder2D(feature_maps=args.fm, levels=args.levels, group_norm=(args.group_norm==1))
else:
    net = Autoencoder3D(feature_maps=args.fm, levels=args.levels, group_norm=(args.group_norm==1))

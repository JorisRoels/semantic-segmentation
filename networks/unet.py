...
class UNetEncoder3D(nn.Module):
    ...
    def __init__(self, in_channels=1, feature_maps=64, levels=4, group_norm=True):
        ...
        # center (lowest) block
        self.center_conv = UNetConvBlock3D(2**(levels-1) * feature_maps, 2**levels * feature_maps, group_norm=group_norm)
...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=2, feature_maps=64, levels=4, group_norm=False, skip_connections=True):
        ...
        # expansive path
        self.decoder = UNetDecoder3D(in_channels, out_channels, feature_maps=feature_maps, levels=levels, skip_connections=skip_connections, group_norm=group_norm)
